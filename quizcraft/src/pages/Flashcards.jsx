  ...
  return (
    <div className="relative max-w-4xl mx-auto mt-8 p-4">
      ...
      <div className="flex flex-col items-center justify-center min-h-[450px]">
        ...
        >
          <div className={`relative w-full h-full transition-transform duration-700 [transform-style:preserve-3d] shadow-[0_0_50px_rgba(0,0,0,0.4)] rounded-3xl ${isFlipped ? '[transform:rotateY(180deg)]' : ''}`}>
        ...
        <div className="flex items-center gap-6 mt-12 w-full max-w-sm justify-between bg-[#1a1333]/50 p-3 rounded-2xl border border-white/5 backdrop-blur-md">
          <button 
            onClick={handlePrev}
            disabled={currentIndex === 0}
            className={`cursor-pointer p-4 rounded-xl flex items-center justify-center transition-all ${currentIndex === 0 ? 'bg-transparent text-gray-600 cursor-not-allowed' : 'bg-white/10 text-white hover:bg-white hover:text-[#0f0a1c] shadow-lg'}`}
          >
            <ChevronLeft size={24} />
          </button>

          <div className="text-center font-black text-gray-300 tracking-widest">
          ...
          <button 
            ...
            className={`cursor-pointer p-4 rounded-xl flex items-center justify-center transition-all ${currentIndex === displayCards.length - 1 ? 'bg-transparent text-gray-600 cursor-not-allowed' : 'bg-white/10 text-white hover:bg-white hover:text-[#0f0a1c] shadow-lg'}`}
          >
